...
*   **進捗の可視化**: 評価セット一覧画面での **進捗率（％）およびプログレスバー** の表示。全153項目のうちどれだけ評価が完了したか一目で把握可能。
...
*   **AIによる判定 & 改善アドバイス**: Google Gemini APIと連携。
    *   **現状判定**: 備考に入力した内容に基づき、AIが「達成・未達成」を客観的に判定。
    *   **具体的な改善案**: ISO 27002 A.5.1等の専門規格に基づき、実務レベルの具体的なアクションを提案。判定結果と改善案は個別に保存されます。
*   **アクションアイテム管理**: 要対応項目から **AIによるタスク案を自動生成** し、担当者や期日を設定して管理。既存タスクの上書き更新にも対応。
*   **ダッシュボード & レポート**:
    *   カテゴリ別の達成状況を円グラフで可視化。
    *   **高品質PDFレポート出力**: 評価結果、備考、およびアクションアイテム一覧をまとめた精細なレポートを生成。
